package com.example.chen.snailweather.view;
...
public class BottomRecyclerView extends RecyclerView implements View.OnTouchListener {
    ...
    public BottomRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    private void scrollItem(int index){
        int dx=0;
        final int tw=BottomRecyclerView.this.getWidth()/7;
        if (index<3){
            dx=-tw*(3-index);
        }else if(index>3){
            dx=tw*(index-3);
        }
        BottomRecyclerView.this.smoothScrollBy(dx,0);
    }
    ...
    public boolean selectNextItem(){
        if (mSelectIndex>= BottomRecyclerView.this.getAdapter().getItemCount()-1){//已经是最后一个
            return false;
        }
        //当前选择的item下落
        ...
        if (lm instanceof MyLayoutManager) {
            ...
            anim.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    mSelectIndex+=1;
                    ((MyLayoutManager) lm).mSelectIndex=mSelectIndex;
                    View view=BottomRecyclerView.this.getChildAt(mSelectIndex-mOffsetIndex);
